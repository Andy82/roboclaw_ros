...
def SpeedAccelDistanceM1M2(address,accel,speed1,distance1,speed2,distance2,buffer):
	return _write4S44S441(address,Cmd.MIXEDSPEEDACCELDIST,accel,speed1,distance1,speed2,distance2,buffer)
...
def ReadEeprom(address,ee_address):
	trys = _trystimeout
	while 1:
		port.flushInput()
		_sendcommand(address,Cmd.READEEPROM)
		crc_update(ee_address)
		port.write(chr(ee_address))
		val1 = _readword()
		if val1[0]:
			crc = _readchecksumword()
			if crc[0]:
...
def WriteEeprom(address,ee_address,ee_word):
	retval = _write111(address,Cmd.WRITEEEPROM,ee_address,ee_word>>8,ee_word&0xFF)
	if retval==True:
		trys = _trystimeout
		while 1:
			port.flushInput()
